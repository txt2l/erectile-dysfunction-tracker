...
function checkEnvironment() {
  ...
  if (!fs.existsSync(envExample)) {
    ...
    const requiredVars = ['DATABASE_URL', 'PORT', 'NODE_ENV', 'JWT_SECRET', 'AWS_REGION', 'GITHUB_CLIENT_ID', 'GITHUB_CLIENT_SECRET'];
    const missingVars = requiredVars.filter(v => !envContent.includes(v));
    if (missingVars.length > 0) {
      fail(`Missing required env vars in .env.example: ${missingVars.join(', ')}`, 'The GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET are required for GitHub OAuth login.');
    } else {
